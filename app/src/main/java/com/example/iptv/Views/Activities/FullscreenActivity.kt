...
class FullscreenActivity : AppCompatActivity(), Player.EventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fs_player.player = simpleExoPlayer
        simpleExoPlayer.seekTo(ps)
...
//            fs_player.resizeMode = AspectRatioFrameLayout.RESIZE_MODE_FIT
    ...
    override fun onPlayerStateChanged(playWhenReady: Boolean, playbackState: Int) {
        if (playbackState == Player.STATE_BUFFERING) {
            fs_loading.visibility = View.VISIBLE
        } else if(playbackState == Player.STATE_READY) {
            fs_loading.visibility = View.GONE
        }
    }

    override fun onBackPressed() {
        val intent = Intent(this@FullscreenActivity, MainActivity::class.java)
        val mPosition = simpleExoPlayer.currentPosition
        intent.putExtra("playbackPosition", mPosition)
        simpleExoPlayer.stop()
        startActivity(intent)
        simpleExoPlayer.release()
    }
